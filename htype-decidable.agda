...
module htype-decidable where
  ...
  lemma-forall : ∀{t1 t2} → ·∀ t1 == ·∀ t2 → t1 == t2
  lemma-forall refl = refl 
  ...
  htype-dec (t1 ==> t2) (·∀ _) = Inr (λ ())

  htype-dec (·∀ _) b = Inr (λ ())
  htype-dec (·∀ _) (T _) = Inr (λ ())
  htype-dec (·∀ _) ⦇-⦈ = Inr (λ ())
  htype-dec (·∀ _) (t2 ==> t3) = Inr (λ ())
  htype-dec (·∀ t1) (·∀ t2) with htype-dec t1 t2 
  ... | Inl refl = Inl refl 
  ... | Inr x = Inr (λ x₁ → x (lemma-forall x₁))
