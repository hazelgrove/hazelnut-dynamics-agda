...
module typed-elaboration where
  mutual
    ...
    typed-elaboration-synth (ESTLam ex) = TATLam (typed-elaboration-synth ex)
    ...
    typed-elaboration-synth (ESTAp x m ex) with typed-elaboration-ana ex 
    ... | con , ih = TATAp (TACast ih con) refl
    ...
    typed-elaboration-ana (EALam x₁ MAArr ex)
      with typed-elaboration-ana ex
    ... | con , D = TCArr ~refl con , TALam x₁ D
    typed-elaboration-ana (EASubsume x x₁ x₂ x₃) = ~sym x₃ , typed-elaboration-synth x₂
    typed-elaboration-ana (EAEHole {Γ = Γ} {u = u}) = ~refl , TAEHole (ctx-top ∅ u (Γ , _) refl) (STAId (λ x τ z → z))
