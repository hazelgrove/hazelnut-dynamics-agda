...
module progress where
  ...
  progress hctxwf (TATAp wf wt eq) | I x | CIFFEHole (_ , _ , _ , _ , _ , _ , refl , _) = I (ITAp (λ t τ1 τ2 d' ()) x)
  progress hctxwf (TATAp wf wt eq) | I x | CIFFNEHole (_ , _ , _ , _ , _ , _ , _ , _ , refl , _) = I (ITAp (λ t τ1 τ2 d' ()) x)
  progress hctxwf (TATAp wf wt eq) | I x | CIFFAp (_ , _ , _ , refl , _) = I (ITAp (λ t τ1 τ2 d' ()) x)
  progress hctxwf (TATAp wf wt eq) | I x | CIFFTApForall (_ , _ , _ , _ , _ , refl , _ ) = I (ITAp (λ t τ1 τ2 d' ()) x)
  progress hctxwf (TATAp wf wt eq) | I x | CIFFTApId (_ , _ , _ , refl , _ ) = I (ITAp (λ t τ1 τ2 d' ()) x)
  progress hctxwf (TATAp wf wt eq) | I x | CIFFCast (a , d , e , _ , refl , f , g , h) = S (_ , Step FHOuter ITTApCast FHOuter)
  
  progress hctxwf (TATAp wf wt eq) | I x | CIFFCastHole (_ , refl , refl , _ ) = I (ITAp (λ t τ1 τ2 d' ()) x)
  progress hctxwf (TATAp wf wt eq) | I x | CIFFFailedCast (_ , _ , refl , refl , _) = I (ITAp (λ t τ1 τ2 d' ()) x)
  ...
  progress hctxwf (TACast wt wf TCHole1) | I x | ·∀ t τ with ground-decidable (·∀ t τ)
  progress hctxwf (TACast wt wf TCHole1) | I x | ·∀ t .⦇-⦈ | Inl GForall = I (ICastGroundHole GForall x)
  progress hctxwf (TACast wt wf TCHole1) | I x₁ | ·∀ t τ | Inr x  = S (_ , Step FHOuter (ITGround (MGForall (ground-forall-not-hole x))) FHOuter)
  ...
  progress hctxwf (TACast wt wf (TCHole2 {b})) | I x | CIFHTApHole (_ , _ , _ , refl , _ )        = I (ICastHoleGround (λ _ _ ()) x GBase)
  progress hctxwf (TACast wt wf (TCHole2 {b})) | I x | CIFHTApId (_ , _ , _ , refl , _ )          = I (ICastHoleGround (λ _ _ ()) x GBase)
  ...
  progress hctxwf (TACast wt wf (TCHole2 {.⦇-⦈ ==> .⦇-⦈})) | I x | Inl GArr | CIFHTApHole (_ , _ , _ , refl , _ )            = I (ICastHoleGround (λ _ _ ()) x GArr)
  progress hctxwf (TACast wt wf (TCHole2 {.⦇-⦈ ==> .⦇-⦈})) | I x | Inl GArr | CIFHTApId (_ , _ , _ , refl , _ )            = I (ICastHoleGround (λ _ _ ()) x GArr)
  ...
  progress hctxwf (TACast wt wf (TCHole2 {·∀ t τ})) | I x
    with ground-decidable (·∀ t τ)
  progress hctxwf (TACast wt wf (TCHole2 {·∀ t ⦇-⦈})) | I x | Inl GForall 
    with canonical-indeterminate-forms-hole wt x
  progress hctxwf (TACast wt wf (TCHole2 {.(·∀ _ ⦇-⦈)})) | I x | Inl GForall | CIFHEHole (_ , _ , _ , _ , _ , _ , refl , _) = I (ICastHoleGround (λ d' τ' ()) x GForall)
  progress hctxwf (TACast wt wf (TCHole2 {.(·∀ _ ⦇-⦈)})) | I x | Inl GForall | CIFHNEHole (_ , _ , _ , _ , _ , _ , _ , _ , refl , _) = I (ICastHoleGround (λ d' τ' ()) x GForall)
  progress hctxwf (TACast wt wf (TCHole2 {.(·∀ _ ⦇-⦈)})) | I x | Inl GForall | CIFHAp (_ , _ , _ , refl , _ ) = I (ICastHoleGround (λ d' τ' ()) x GForall)
  progress hctxwf (TACast wt wf (TCHole2 {.(·∀ _ ⦇-⦈)})) | I x | Inl GForall | CIFHTApHole (_ , _ , _ , refl , _ ) = I (ICastHoleGround (λ d' τ' ()) x GForall)
  progress hctxwf (TACast wt wf (TCHole2 {.(·∀ _ ⦇-⦈)})) | I x | Inl GForall | CIFHTApId (_ , _ , _ , refl , _ )   = I (ICastHoleGround (λ d' τ' ()) x GForall)
  progress hctxwf (TACast wt wf (TCHole2 {.(·∀ _ ⦇-⦈)})) | I x | Inl GForall | CIFHCast (_ , ._ , refl , _ , GBase , _) = S (_ , Step FHOuter (ITCastFail GBase GForall (λ ())) FHOuter )
  progress hctxwf (TACast wt wf (TCHole2 {.(·∀ _ ⦇-⦈)})) | I x | Inl GForall | CIFHCast (_ , ._ , refl , _ , GArr , _) = S (_ , Step FHOuter (ITCastFail GArr GForall (λ ())) FHOuter)
  progress hctxwf (TACast wt wf (TCHole2 {(·∀ t ⦇-⦈)})) | I x | Inl GForall | CIFHCast (_ , ._ , refl , _ , GForall {t = t'} , _) with natEQ t t' 
  progress hctxwf (TACast wt wf (TCHole2 {.(·∀ _ ⦇-⦈)})) | I x | Inl GForall | CIFHCast (_ , .(·∀ _ ⦇-⦈) , refl , _ , GForall {_} , _) | Inl refl = S (_ , Step FHOuter (ITCastSucceed GForall) FHOuter)
  progress hctxwf (TACast wt wf (TCHole2 {.(·∀ _ ⦇-⦈)})) | I x | Inl GForall | CIFHCast (_ , .(·∀ _ ⦇-⦈) , refl , _ , GForall {_} , _) | Inr neq = S (_ , Step FHOuter (ITCastFail GForall GForall λ x₁ → neq (forall-inj1 (sym x₁))) FHOuter)
  progress hctxwf (TACast wt wf (TCHole2 {·∀ _ τ})) | I x₁ | Inr x = S (_ , Step FHOuter (ITExpand (MGForall (ground-forall-not-hole x))) FHOuter)
    -- if both are arrows
  ...
  progress hctxwf (TACast wt wf (TCArr c1 c2)) | I x₁ | Inr x = I (ICastArr x x₁)
    -- if both are foralls 
  progress hctxwf (TACast wt wf (TCForall {t} {τ1} {τ2} con)) | I x
    with htype-dec (·∀ t τ1) (·∀ t τ2) 
  ...
  progress hctxwf (TACast wt wf (TCHole1 {·∀ t τ})) | BV x₁ | Inr x 
    with (htype-dec  (·∀ t τ) (·∀ t ⦇-⦈))
  progress hctxwf (TACast wt wf (TCHole1 {·∀ _ .⦇-⦈})) | BV x₁ | Inr x | Inl refl = BV (BVHoleCast GForall x₁)
  progress hctxwf (TACast wt wf (TCHole1 {·∀ _ τ})) | BV x₁ | Inr x₂ | Inr x = S (_ , Step FHOuter (ITGround (MGForall x)) FHOuter) 
    -- if right is hole
  ...
  progress {τ = ·∀ t τ} hctxwf (TACast wt wf TCHole2) | BV x₃ | _ , _ , refl , _ , _ | Inr _ | Inr x | Inr (Inr (Inr (_ , (_ , refl)))) = S(_ , Step FHOuter (ITExpand (MGForall (ground-forall-not-hole x))) FHOuter )
    -- if both arrows
  ...
  progress hctxwf (TACast wt wf (TCForall {t} {τ1} {τ2} _)) | BV x
    with htype-dec (·∀ t τ1) (·∀ t τ2)
  progress hctxwf (TACast wt wf (TCForall {t} {τ1} {τ2} _)) | BV x₁ | Inl refl = S (_ , Step FHOuter ITCastID FHOuter)
  progress hctxwf (TACast wt wf (TCForall {t} {τ1} {τ2} _)) | BV x₁ | Inr x = BV (BVForallCast (λ {e → x ((forall-lemma e))}) x₁)
