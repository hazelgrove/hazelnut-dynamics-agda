...
--  canonical-indeterminate-forms-base TAConst ()
    ...
    CIFFTApForall : ∀ {Δ d t τ} →
      Σ[ d1 ∈ ihexp ] Σ[ t ∈ Nat ] Σ[ τ ∈ htyp ] Σ[ t' ∈ Nat ] Σ[ τ' ∈ htyp ] 
        ((d == d1 < τ >) ×
         (Δ , ∅ , ∅ ⊢ d1 :: ·∀ t (·∀ t' τ')) ×
         (d1 indet) ×
         ((τ2 τ2' : htyp) (t2 : Nat) (d1' : ihexp) → d1 ≠ (d1' ⟨(·∀ t2 τ2) ⇒ (·∀ t2 τ2')⟩))
        )
        → cif-forall Δ d t τ
    CIFFTApId : ∀ {Δ d t τ} →
      Σ[ d1 ∈ ihexp ] Σ[ t' ∈ Nat ] Σ[ τ1 ∈ htyp ]
        ((d == d1 < ·∀ t' τ1 >) ×
         (Δ , ∅ , ∅ ⊢ d1 :: ·∀ t (T t)) ×
         (d1 indet) ×
         ((τ3 τ3' : htyp) (t3 : Nat) (d1' : ihexp) → d1 ≠ (d1' ⟨(·∀ t3 τ3) ⇒ (·∀ t3 τ3')⟩))
        )
        → cif-forall Δ d t τ
    CIFFCast : ∀{d Δ t τ} →
  ...
  ... | Inl refl rewrite eq = CIFHTApId (_ , _ , τ1 , refl , wt , ind , x) -- ( _ , (τ1 , (refl , (wt , ind , x))) )
  ... | Inr neq rewrite eq = CIFHTApHole (_ , _ , τ1 , refl , wt , ind , x)
