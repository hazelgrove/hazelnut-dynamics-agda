...
module canonical-indeterminate-forms where
  ...
  data cif-base : (Δ : hctx) (d : ihexp) → Set where
    CIFBEHole : ∀ {Δ d} →
      Σ[ u ∈ Nat ] Σ[ θ ∈ typenv ] Σ[ σ ∈ env ] Σ[ Θ ∈ typctx ] Σ[ Γ ∈ tctx ]
        ((d == ⦇-⦈⟨ u , θ , σ ⟩) ×
         ((u :: b [ Θ , Γ ]) ∈ Δ) ×
         (Δ , ~∅ , ∅ ⊢ θ , σ :s: Θ , Γ)
        )
       → cif-base Δ d
    CIFBNEHole : ∀ {Δ d} →
      Σ[ u ∈ Nat ] Σ[ θ ∈ typenv ] Σ[ σ ∈ env ] Σ[ Θ ∈ typctx ] Σ[ Γ ∈ tctx ] Σ[ d' ∈ ihexp ] Σ[ τ' ∈ htyp ]
        ((d == ⦇⌜ d' ⌟⦈⟨ u , θ , σ ⟩) ×
         (Δ , ~∅ , ∅ ⊢ d' :: τ') ×
         (d' final) ×
         ((u :: b [ Θ , Γ ]) ∈ Δ) ×
...
--  canonical-indeterminate-forms-base TAConst ()
  ...
  canonical-indeterminate-forms-base (TAEHole x x₁) IEHole = CIFBEHole (_ , _ , _ , _ , _ , refl , x , x₁)
  canonical-indeterminate-forms-base (TANEHole x wt x₁) (INEHole x₂) = CIFBNEHole (_ , _ , _ , _ , _ , _ , _ , refl , wt , x₂ , x , x₁)
  ...
  data cif-arr : (Δ : hctx) (d : ihexp) (τ1 τ2 : htyp) → Set where
    ...
    CIFANEHole : ∀{d Δ τ1 τ2} →
      Σ[ u ∈ Nat ] Σ[ θ ∈ typenv ] Σ[ σ ∈ env ] Σ[ d' ∈ ihexp ] Σ[ τ' ∈ htyp ] Σ[ Θ ∈ typctx ] Σ[ Γ ∈ tctx ]
        ((d == ⦇⌜ d' ⌟⦈⟨ u , θ , σ ⟩) ×
         (Δ , ~∅ , ∅ ⊢ d' :: τ') ×
         (d' final) ×
         ((u :: (τ1 ==> τ2) [ Θ , Γ ]) ∈ Δ) ×
  ...
  canonical-indeterminate-forms-arr (TAEHole x x₁) IEHole = CIFAEHole (_ , _ , _ , _ , _ , refl , x , x₁)
  canonical-indeterminate-forms-arr (TANEHole x wt x₁) (INEHole x₂) = CIFANEHole (_ , _ , _ , _ , _ , _ , _ , refl , wt , x₂ , x , x₁)
  ...
  canonical-indeterminate-forms-forall (TAEHole x x₁) IEHole = CIFFEHole (_ , _ , _ , _ , _ , refl , x , x₁)
  canonical-indeterminate-forms-forall (TANEHole x wt x₁) (INEHole x₂) = CIFFNEHole (_ , _ , _ , _ , _ , _ , _ , refl , wt , x₂ , x , x₁)
  ...
  canonical-indeterminate-forms-hole (TAEHole x x₁) IEHole = CIFHEHole (_ , _ , _ , _ , _ , refl , x , x₁)
  canonical-indeterminate-forms-hole (TANEHole x wt x₁) (INEHole x₂) = CIFHNEHole (_ , _ , _ , _ , _ , _ , _ , refl , wt , x₂ , x , x₁)
