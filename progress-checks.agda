...
module progress-checks where
  ...
  boxedval-not-step (BVArrCast x bv) (d0' , Step FHOuter (ITCastID eq) FHOuter) = x eq
  boxedval-not-step (BVArrCast x bv) (_ , Step (FHCast x₁) x₂ (FHCast x₃)) = boxedval-not-step bv (_ , Step x₁ x₂ x₃)
  boxedval-not-step (BVForallCast x bv) (_ , Step FHOuter (ITCastID eq) FHOuter) = x eq
  boxedval-not-step (BVForallCast x bv) (_ , Step (FHCast x₁) x₂ (FHCast x₃)) = boxedval-not-step bv (_ , Step x₁ x₂ x₃)
  boxedval-not-step (BVHoleCast GBase bv) (d' , Step FHOuter (ITCastID ()) FHOuter)
  boxedval-not-step (BVHoleCast GArr bv) (d' , Step FHOuter (ITCastID ()) FHOuter)
  boxedval-not-step (BVHoleCast GForall bv) (d' , Step FHOuter (ITCastID ()) FHOuter)
  ...
  mutual
    ...
    indet-not-step (ITAp x ind) (_ , Step FHOuter (ITTApCast) FHOuter) = x _ _ _ _ _ refl
    indet-not-step (ITAp x ind) (_ , Step (FHTAp x₂) x₃ (FHTAp x₄)) = indet-not-step ind (_ , Step x₂ x₃ x₄)
    indet-not-step (ICastArr x ind) (d0' , Step FHOuter (ITCastID eq) FHOuter) = x eq
    indet-not-step (ICastArr x ind) (_ , Step (FHCast x₁) x₂ (FHCast x₃)) = indet-not-step ind (_ , Step x₁ x₂ x₃)
    indet-not-step (ICastForall x ind) (d0' , Step FHOuter (ITCastID eq) FHOuter) = x eq
    indet-not-step (ICastForall x ind) (_ , Step (FHCast x₁) x₂ (FHCast x₃)) = indet-not-step ind (_ , Step x₁ x₂ x₃)
    indet-not-step (ICastGroundHole GBase ind) (d' , Step FHOuter (ITCastID ()) FHOuter)
    indet-not-step (ICastGroundHole GArr ind) (d' , Step FHOuter (ITCastID ()) FHOuter)
    indet-not-step (ICastGroundHole GForall ind) (d' , Step FHOuter (ITCastID ()) FHOuter)
    ...
    indet-not-step (ICastHoleGround x ind GBase) (d' , Step FHOuter (ITCastID ()) FHOuter)
    indet-not-step (ICastHoleGround x ind GArr) (d' , Step FHOuter (ITCastID ()) FHOuter)
    indet-not-step (ICastHoleGround x ind GForall) (d' , Step FHOuter (ITCastID ()) FHOuter)
    indet-not-step (ICastHoleGround x ind g) (d' , Step FHOuter (ITCastSucceed g1 g2 eq) FHOuter) = x _ _ refl
