...
module debruijn.debruijn-parametricity where
  ...
  ... | Inl refl rewrite ttSubVar {n} {m} {d1} rewrite ttSubVar {n} {m} {d1'} = {!   !} 
  ... | Inr neq = Eq0Var
  ...
  -- eq0-subst' {x} (·λ[ t3 ] d3) (·λ[ t4 ] d4) eq0 (Eq0Lam eq0') = Eq0Lam {!   !} --Eq0Lam (eq0-subst' d3 d4 eq0 eq0')
  -- eq0-subst' (·Λ d3) (·Λ d4) eq0 (Eq0TLam eq0') = {!   !} --Eq0TLam (eq0-subst' d3 d4 eq0 eq0')
  ...
  -- eq0-subst' (d3 ⟨ t3 ⇒ t3' ⟩) (d4 ⟨ t4 ⇒ t4' ⟩) eq0 (Eq0Cast eq0') = Eq0Cast (eq0-subst' d3 d4 eq0 eq0')
  ...
  -- eq0-typsubst' (·λ[ x₁ ] d1) (·λ[ _ ] d2) (DCLam compl _) wf1 wf2 (Eq0Lam eq0) = Eq0Lam (eq0-typsubst' d1 d2 compl wf1 wf2 eq0)
  ...
  -- eq0-typsubst' (d1 ⟨ x ⇒ x' ⟩) (d2 ⟨ x₂ ⇒ x₂' ⟩) (DCCast compl t1compl t2compl alpha) wf1 wf2 (Eq0Cast eq0) = Eq0Cast (eq0-typsubst' d1 d2 compl wf1 wf2 eq0) (alpha-sub wf1 (lemma-alpha-forall alphal)) (alpha-sub wf2 (lemma-alpha-forall alphar))
  ...
  -- eq0-instep' {d1 = (·λ[ τ1 ] d1) ∘ d1'} {d2 = (·λ[ τ2 ] d2) ∘ d2'} (Eq0Ap (Eq0Lam eq0) eq1) ITLam compl wt1 wt2 = _ , ITLam , {!  eq0-ttSub eq1 eq0 !} --([ d2' / x1 ] d2) , ITLam , eq0-subst' d1 d2 eq1 eq0
  ...
  -- eq0-instep' {d1 = ·Λ d1 < _ >} {d2 = ·Λ d2 < τ2 >} (Eq0TAp (Eq0TLam eq0)) ITTLam (DCTAp x (DCTLam compl)) (TATAp x₁ (TATLam wt1) x₂) (TATAp x₃ (TATLam wt2) x₄) = {!   !} --[ τ2 / t ] d2 , ITTLam , ? --eq0-typsubst' d1 d2 compl x₁ x₃ eq0
  -- eq0-instep' {d1 = (d1 ⟨ ·∀ τ ⇒ ·∀ τ' ⟩) < τ1 >} {d2 = (d2 ⟨ ·∀ τ2 ⇒ ·∀ τ2' ⟩) < τ3 >} 
  --   (Eq0TAp (Eq0Cast eq0)) ITTApCast compl (TATAp x₅ (TACast wt1 x₇ x₈) x₆) (TATAp x (TACast wt x₂ x₃) x₁) = {!   !}
  --     -- (d2 < τ3 >) ⟨ TT[ τ3 / t2 ] τ2 ⇒ TT[ τ3 / t2' ] τ2' ⟩ , ITTApCast , Eq0Cast (Eq0TAp eq0) 
  ...
  -- ... | c1' | c2'
  --   with parametricity11_rec (compl-wt-complid c1' {!   !}) (compl-wt-complid c2' {!   !}) {!   !} {!   !} eq2 step bv
  ...
  --   let (v2 , v2eval , v2bv , eq0') = parametricity11_rec d1c d2c (typed-elaboration-syn {!   !} elab1) (typed-elaboration-syn {!   !} elab2) (eq0-eq0' d1c d2c (eq0-elab-syn eeq elab1 elab2)) eval bv in
