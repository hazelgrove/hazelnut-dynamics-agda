...
module debruijn.debruijn-lemmas-wf where
  ...
  wf-ctx-var (CtxWFVar x ctxwf) InCtxZ = {!  !}
  wf-ctx-var (CtxWFVar x ctxwf) (InCtx1+ inctx) = {!   !}
  wf-ctx-var (CtxWFTVar ctxwf) (InCtxSkip inctx) = {!   !}
  ...
  wf-inc {m = Z} (WFSkip wf) = WFVarS (WFSkip wf)
  wf-inc {m = 1+ m} (WFSkip wf) = WFVarZ
  ...
  wf-syn ctxwf (SLam x syn) = WFArr x (wf-syn ctxwf {!   !})
  wf-syn ctxwf (STLam syn) = WFForall (wf-syn (CtxWFTVar ctxwf) {!   !})
  ...
  wf-elab-syn ctxwf (ESVar x) = {!   !}
  wf-elab-syn ctxwf (ESLam x syn) = WFArr x (wf-elab-syn ctxwf {!   !})
  wf-elab-syn ctxwf (ESTLam syn) = WFForall (wf-elab-syn (CtxWFTVar ctxwf) {!   !})
  ...
  wf-ta ctxwf (TAVar x) = {!   !}
  wf-ta ctxwf (TALam x wt) = WFArr x {! wf-ta ? wt  !} --WFArr x (wf-ta ? wt)
  wf-ta ctxwf (TATLam wt) = {!   !} --WFForall (wf-ta ? wt)
