...
module typed-expansion where
  ...
  lem-subweak : ∀{Δ Γ Γ' Δ' σ} → Δ ## Δ' → Δ , Γ ⊢ σ :s: Γ' → (Δ ∪ Δ') , Γ ⊢ σ :s: Γ'
  lem-subweak disj sub x d xd∈σ with sub x d xd∈σ
  lem-subweak disj sub x d xd∈σ | π1 , π2 , π3 = {!!}
  ...
  lem-weakenΔ1 d (TAEHole {Δ = Δ} x y) = TAEHole (x∈∪l Δ _ _ _ x) (lem-subweak {!!} y)
  lem-weakenΔ1 d (TANEHole {Δ = Δ} D x y) = TANEHole (x∈∪l Δ _ _ _ D) (lem-weakenΔ1 d x) (lem-subweak {!!} y)
