...
module lemmas-tysubst-ta where
  ...
  mutual
    ...
    lemma-tysubst-subst {Γ = Γ} wf ubd ubg tub (TUBσSubst x₁ ubs) bd (TBUσSubst x₂ bu x₃) (STAIdSubst ts x alpha) = 
      STAIdSubst (rewrite-gamma-subst (lem-map-extend-dist {Γ = Γ}) 
        (lemma-tysubst-subst wf ubd (lem-ub-extend (tunbound-ta-tunboundt x₁ ubd ubg x) ubg) tub ubs {!   !} bu {! ts !})) 
        (lemma-tysubst wf tub {!   !} {!   !} ubd ubg x₁ x₂ x) {!   !}
    lemma-tysubst-subst {Θ = Θ} {σ = σ} wf ubd ubg tub ubs (BDTθSubst x₁ bd) bu (STASubst ts x) =
      {!   !} -- STASubst (STASubst ({! lemma-tysubst-subst  !}) x) wf
    ...
    lemma-tysubst {Γ = Γ} wf tub tbdt (TBDTLam tbd tbtd) ubd ubg (TUBLam2 ub ub') (TBULam bu) (TALam x x₁ ta) = 
      ...
      (wf-sub (tbinderstt-disjoint-sym tbtd) (wf-closed wf tbdt) x₁ refl) -- (wf-closed wf) x₁ refl)
    ...
    ... | Inl refl rewrite natEQrefl {t'} rewrite natEQrefl {t} rewrite ! eq = TATAp {t = t'} {τ2 = τ4} (wf-sub ((tbinderstt-disjoint-sym x₁)) (wf-closed wf tbdt) x refl)
    ...
    ...   | UBForall _ ub4 =
       TATAp {t = t'} {τ2 = Typ[ τ2 / t ] τ4} (wf-sub (tbinderstt-disjoint-sym x₁) (wf-closed wf tbdt) x refl)
    ...
    lemma-tysubst wf tub tbdt (TBDTCast tbd x₃ x₄) ubd ubg (TUBCast ub x₁ x₂) (TBUCast bu bd1 bd2) (TACast ta x x~ alpha) = TACast (lemma-tysubst wf tub tbdt tbd ubd ubg ub bu ta) (wf-sub x₄ (wf-closed wf tbdt) x refl) (~Typ[] wf x₁ x₂ x~) (alpha-sub wf (lemma-alpha-forall alpha))
